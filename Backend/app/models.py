...
class User(db.Model, UserMixin):
    ...
    favorites = db.relationship('Recipe', secondary=favorites, backref=db.backref('favorited_users', lazy=True), lazy='dynamic')
    recipes = db.relationship(
        'Recipe', secondary=user_recipes, backref=db.backref('users', lazy='dynamic'))
...
class Recipe(db.Model):
    ...
    latest_users = db.relationship('User', secondary=latest_users, backref=db.backref(
        'latest_added_recipes', lazy='dynamic'), lazy='dynamic')
    api_recipe = db.Column(db.Boolean, default=False)
    favorited_by = db.relationship('User', secondary=favorites,
        backref=db.backref('favorite_recipes', lazy=True),
